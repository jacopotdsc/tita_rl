...
# cusadi
sys.path.insert(0, os.path.join(ROOT_DIR, "test", "cusadi"))
from cusadi import CusadiFunction # noqa: E402  (needs sys.path set first)
import casadi as ca

class LeggedMPCRobot(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        casadi_dir = os.path.join(curent_dir, "..", "test", "casadi_functions")
        self.f_dynamics = self.load_fn(casadi_dir, "f_dynamics")
        self.f_constraints = self.load_fn(casadi_dir, "f_constraints")
        self.f_rollout = self.load_fn(casadi_dir, "f_rollout")
    ...
    # ----------- loading casadi -------------------------
    def load_fn(self, casadi_dir, name):
        path = os.path.join(casadi_dir, f"{name}.casadi")
        if not os.path.exists(path):
            print(f"[ERROR] CasADi function file not found: {path}")
            sys.exit(1)
        fn = ca.Function.load(path)
        fn_compiled = CusadiFunction(fn, 1)
        print(f"[LOADED] CasADi function {name} from {path}")
        return fn_compiled
    ...
    def step(self, actions):
        ...
        MASS = 27.0
        G = 9.81
        x = torch.zeros(13, device=DEVICE, dtype=DTYPE)
        x[2] = 0.44
        x[8] = 0.44
        x[3:6] = torch.randn(3, device=DEVICE, dtype=DTYPE) * 0.1

        # input singolo (ignori actions come hai detto)
        u = torch.zeros(8, device=DEVICE, dtype=DTYPE)
        u[5] = 27.0 * 9.81 / 2.0
        u[7] = 27.0 * 9.81 / 2.0   

        # CasADi rollout
        out = self.f_rollout.evaluate([x, u])
